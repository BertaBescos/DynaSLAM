...
cv::Mat System::TrackMonocular(const cv::Mat &im, const cv::Mat &mask, const double &timestamp, const cv::Mat &im_dyn)
{
    if(mSensor!=MONOCULAR)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonocular(im,mask,timestamp, im_dyn);
